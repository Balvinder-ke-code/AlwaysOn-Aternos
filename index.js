 ...
 * Put values in Render "Environment" tab.
 * Anything not provided will fall back to the defaults below.
 */
...
async function startAternosIfConfigured() {
  ...
  try {
    ...
    // Aternos sometimes returns non-JSON; handle safely
    let data = null;
...
function startBot() {
  ...
  function scheduleReconnect(reasonLabel) {
    ...
    reconnecting = true;
    const delay = RECONNECT_DELAY + Math.floor(Math.random() * 20000); // jitter
    console.log(`🔄 Reconnecting in ${Math.round(delay / 1000)}s... (${reasonLabel})`);
    setTimeout(async () => {
      reconnecting = false;
      if (reasonLabel.includes("throttled") || reasonLabel.includes("end") || reasonLabel.includes("error")) {
        // Optionally trigger Aternos start each retry
  ...
  bot.on("kicked", (reason) => {
    ...
    if (msg.includes("Connection throttled")) {
      scheduleReconnect("throttled");
    } else if (msg.includes("duplicate_login")) {
      // Give server time to clear ghost session
      setTimeout(() => scheduleReconnect("duplicate_login"), RECONNECT_DELAY * 2);
    } else if (msg.includes("idling")) {
      // If you get kicked for idling, we still reconnect
      scheduleReconnect("idling");
    } else {
...
// —— Start flow ——
// Try to start Aternos first (if configured), then connect bot.
const init = async () => {
  await startAternosIfConfigured();
  startBot();
};
